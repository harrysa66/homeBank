...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`homebank`@`localhost` SQL SECURITY DEFINER VIEW `vbuget` AS (select `t1`.`id` AS `id`,`t1`.`month` AS `month`,`t1`.`value` AS `value`,`t1`.`unit` AS `unit`,`t2`.`codename` AS `unitname` from (`buget` `t1` left join `datadict` `t2` on(((`t1`.`unit` = `t2`.`code`) and (`t2`.`catalog` = 'currency'))))) */;
